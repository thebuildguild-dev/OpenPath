...
const difficultyConfig = {
  ...
  medium: { label: 'Medium', color: 'text-accent-sunset-soft', bg: 'bg-accent-sunset/10' },
  hard:   { label: 'Hard',   color: 'text-red-400',            bg: 'bg-red-400/10' },
};

function ScoreBar({ score }: { score: number }) {
  const pct = Math.min(100, Math.max(0, score));
  const color = pct >= 70 ? '#6ee7b7' : pct >= 40 ? '#ff7a17' : '#f87171';
  ...
  return (
    <div className="card">
      ...
      <div className="flex items-start justify-between gap-4 mb-6">
        ...
        <span className={clsx('font-mono text-[11px] uppercase tracking-[0.08em] px-3 py-1.5 rounded-full', cfg.color, cfg.bg)}>
          {cfg.label}
        </span>
      </div>
          ...
          <div className="space-y-2">
            {commands.map(([key, val]) => (
              <div key={key} className="flex items-center gap-3">
                ...
                <div className="flex-1 flex items-center gap-2 bg-canvas-soft rounded-[6px] px-3 py-2">
        ...
        <div className="mb-6">
          ...
          <div className="space-y-2">
            {setup.blockers.map((b, i) => (
              <div key={i} className="flex items-start gap-2.5 bg-accent-sunset/8 rounded-[8px] px-3 py-2.5">
      ...
      {setup.recommendations.length > 0 && (
        <div>
          ...
          <div className={clsx('space-y-2 overflow-hidden transition-all duration-300', showAll ? 'max-h-96' : 'max-h-0')}>
            {setup.recommendations.map((r, i) => (
              <div key={i} className="flex items-start gap-2.5 bg-canvas-soft rounded-[8px] px-3 py-2.5">
